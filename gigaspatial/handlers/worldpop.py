...
class WorldPopConfig(BaseModel):
    ...
    SCHOOL_AGE_POPULATION_PATH: str = Field(default="GIS/AgeSex_structures/school_age_population/v1/2020/")
    PPP_2021_2022_PATH: str = Field(default="GIS/Population/Global_2021_2022_1km_UNadj/")
    DATASETS_METADATA_PATH: str = Field(default="assets/wpgpDatasets.csv")

    # user config
    base_path: Path = Field(default=config.get_path("worldpop", "bronze"))
    country: str = Field(...)
    ...
    def validate_configuration(self):
        ...
        if self.year > self.SCHOOL_AGE_YEAR:
            if self.resolution != "LOW":
                raise ValueError(f"Data for year {self.year} is only available at LOW (1km) resolution")
                
            if not self.un_adjusted:
                raise ValueError(f"Data for year {self.year} is only available with UN adjustment")
        
        if self.school_age:
            if self.resolution != "LOW":
                raise ValueError(f"School age data is only available at LOW (1km) resolution")
    ...
    def dataset_path(self) -> Path:
        ...
        file_path = '/'.join([url_parts[4], url_parts[5], url_parts[7], self.country, url_parts[-1]]) if self.school_age else '/'.join([url_parts[4], url_parts[6], self.country, url_parts[-1]])
    ...
    def _load_datasets_metadata(self) -> pd.DataFrame:
        ...
        try:
            WorldPopConfig._metadata_cache = pd.read_csv(str(self.WORLDPOP_DB_BASE_URL) + self.DATASETS_METADATA_PATH)
            return WorldPopConfig._metadata_cache
    ...
    def _compute_dataset_url(self) -> str:
        ...
        if self.year > self.SCHOOL_AGE_YEAR:
            return str(self.WORLDPOP_DB_BASE_URL) + self.PPP_2021_2022_PATH + f"{'' if self.constrained else 'un'}constrained/{self.year}/{self.country}/{self.country.lower()}_ppp_{self.year}_1km_UNadj{'_constrained' if self.constrained else ''}.tif"
        
        # handle school-age population datasets
        if self.school_age:
            return str(self.WORLDPOP_DB_BASE_URL) + self.SCHOOL_AGE_POPULATION_PATH + f"{self.country}/{self.country}_SAP_1km_2020/{self.country}_{self.gender}_{self.school_age}_2020_1km.tif"

        # handle standard population datasets
        wp_metadata = self._load_datasets_metadata()
        
        try:
            dataset_url = self.WORLDPOP_DB_BASE_URL + wp_metadata[
                (wp_metadata.ISO3 == self.country) & 
                (wp_metadata.Covariate == 'ppp_' + str(self.year) + ('_UNadj' if self.un_adjusted else ''))
            ].PathToRaster.values[0]
        except IndexError:
            raise ValueError(f"No dataset found for country={self.country}, year={self.year}, un_adjusted={self.un_adjusted}")
        ...
        url_parts[5] = url_parts[5] + '_1km' + ('_UNadj' if self.un_adjusted else '')  # get 1km folder with UNadj specification
        url_parts[8] = url_parts[8].replace(str(self.year), str(self.year) + '_1km_Aggregated')  # get filename with 1km res
